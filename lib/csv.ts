...
export function toCSV(loans: Loan[]): string {
  ...
  for (const l of loans) {
    const record: Record<string, string> = {
      ...
      principalPerMonth: String(l.principalPerMonth ?? ""),
      bonusPayment: String(l.bonusPayment ?? ""),
      bonusMonths: JSON.stringify(l.bonusMonths ?? []),
      revolvingRate: String(l.revolvingRate ?? ""),
      revolvingMin: String(l.revolvingMin ?? ""),
      charges: JSON.stringify(l.charges ?? []),
...
export function fromCSV(csv: string): Loan[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    loans.push({
      ...
      repayType: (["revolving", "principal_equal"].includes(obj.repayType) ? obj.repayType : "fixed") as "fixed" | "principal_equal" | "revolving",
      principalPerMonth: obj.principalPerMonth ? Number(obj.principalPerMonth) : undefined,
      bonusPayment: obj.bonusPayment ? Number(obj.bonusPayment) : undefined,
      bonusMonths: parseJSONArray<number>(obj.bonusMonths),
      ...
      charges: parseJSONArray(obj.charges),
